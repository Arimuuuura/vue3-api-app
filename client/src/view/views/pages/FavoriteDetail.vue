    ...
    <div class="favorite-detail__card">
      ...
      <div>
        ...
        <dl class="favorite-detail__card-body">
          ...
          <dd class="favorite-detail__card-value">
            {{ favoriteItem.shopName }}
          </dd>

          <dt class="favorite-detail__card-key">レビュー平均</dt>
          <dd class="favorite-detail__card-value">
            {{ favoriteItem.reviewAverage }}
          </dd>

          <dt class="favorite-detail__card-key">レビュー件数</dt>
          <dd class="favorite-detail__card-value">
            {{ favoriteItem.reviewCount }}
          </dd>
        </dl>
        <div class="favorite-detail__card-footer">
          <p class="favorite-detail__card-footer-price">
            ポイント {{ favoriteItem.pointRate }} 倍
          </p>
          <button
            class="favorite-detail__card-footer-favorite"
            @click="onClickDeleteFavorite"
          >
            お気に入りに登録解除
          </button>
          <a
            :href="favoriteItem.itemUrl"
            class="favorite-detail__card-footer-link"
          >
            購入ページへ
          </a>
        </div>
...
export default defineComponent({
  setup() {
    ...
    const favoriteItem = computed(() => {
      return store.state.FavoriteState.favoriteItems.find(
        (item) => item.bookmarkId === selectedItemId.value
      )
    })
